    ...
    useEffect(() => {
        const fetchProducts = async (customerLat = null, customerLon = null) => {
            try {
                ...
                // ✅ ADD DISTANCE IF NOT ALREADY INCLUDED BY BACKEND
                const addDistance = (productList) => {
                    if (customerLat && customerLon) {
                        return productList.map(product => {
                            if (product.lat && product.lon) {
                                const distance = getDistanceFromLatLonInKm(customerLat, customerLon, product.lat, product.lon);
                    ...
                    return productList.map(p => ({...p, distance: 'Location not available', distanceValue: 999}));
                };
        ...
        // Get customer location first, then fetch products
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    const userLat = position.coords.latitude;
                    const userLon = position.coords.longitude;
                    fetchProducts(userLat, userLon);
                },
                (error) => {
                    console.warn(`Geolocation error: ${error.message}. Fetching all products.`);
                    fetchProducts(); // Fetch all products if location not available
                }
            );
        } else {
            console.warn("Geolocation is not supported by this browser. Fetching all products.");
            fetchProducts(); // Fetch all products if geolocation not supported
        }
